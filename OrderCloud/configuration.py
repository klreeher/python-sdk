...
class Impersonation(object):
    def start(self,access_token=None):
        ...
        if(access_token == None and config._impersonateToken == None):
...
class Auth(object):
    ...
    def login(self, user,password,client_secret=None):
        ...
        config = Configuration()
        body = "client_id="+config.client_id+"&grant_type=password&username="+user+"&password="+password+"&scope="+"+".join(config.scopes)
        if(client_secret != None):
            body += "&client_secret="+client_secret
        urllib3.disable_warnings()
        pool = urllib3.PoolManager()
        resp = pool.urlopen("POST",self.tokenurl,body=body)
    ...
    def authenticate(self,client_secret=None):
        ...
        config = Configuration()
        body = "client_id="+config.client_id+"&grant_type=client_credentials&scope="+"+".join(config.scopes)
        if(client_secret != None):
            body += "&client_secret="+client_secret
        urllib3.disable_warnings()
        pool = urllib3.PoolManager()
        resp = pool.urlopen("POST",self.tokenurl,body=body)
        if(resp.data == b''):
            raise ValueError("Empty response! Make sure you've set a Default User Context ID in the developer center. This is because the generated access token needs to associated with some user.")
        data = json.loads(resp.data.decode('utf-8'))
        if('error' in data):
            raise ValueError(
                data['error']
